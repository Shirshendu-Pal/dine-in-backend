...
class AuthController extends Controller {
  ...
  public function login(Request $request) {
    ...
    if (!AppUser::where('password', $password)->exists()) {
      return response()->json(['message' => 'Password does not exist'], 409);
    }

    $user = AppUser::where('email', $email)->first();

    if ($user->password != $password) {
      return response()->json(['message' => 'Password is incorrect'], 409);
    }
